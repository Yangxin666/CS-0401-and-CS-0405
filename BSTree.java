...
class BSTree<T>
{
	...
	private boolean containsHelper(BSTNode<T> r,T key)
	{
		...
		else if (((Comparable)key).compareTo(r.getKey())<0)
		{
			return containsHelper(r.getLeft(),key);
		}
		else if (((Comparable)key).compareTo(r.getKey())>0)
		{
			return containsHelper(r.getRight(),key);
		}
		return false;
	}
