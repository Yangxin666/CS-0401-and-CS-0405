...
{	
	...
	public boolean add( T element ) // O(n)
	...
	public void clear()
	{
		count=0; // SIMPLE 1 LINER, TOTALLY SHALLOW/LOGICAL, not physically deleted
	}
	...
	private void upSize()
	{ 
		T[] UpsizedArray = (T[]) new Object[size()*2];
		for (int i=0;i<size();i++)
		{
			UpsizedArray[i]=get(i);
		}
		theArray=UpsizedArray;
	}
	...
	{	
		ArrBag<T> NewBag=new ArrBag<T>();
		for (int i=0;i<size();i++)
		{
			if (!NewBag.contains(get(i)))	NewBag.add(get(i));
		}
		for (int j=0;j<other.size();j++)
		{
			if (!NewBag.contains(other.get(j)))	NewBag.add(other.get(j));
		}
		return NewBag;
	}
	...
	public ArrBag<T> intersection( ArrBag<T> other ) // O(n^3)
	{
		ArrBag<T> NewBag=new ArrBag<T>();
		for (int i=0;i<other.size();i++)
		{
			if (contains(other.get(i)) && !NewBag.contains(other.get(i)))	NewBag.add(other.get(i));
		}
		return NewBag; 
	}
	...
	public ArrBag<T> difference( ArrBag<T> other )
	{
		...
		for (int i=0;i<size();i++)
		{
			if (!other.contains(get(i)) && !NewBag.contains(get(i)))	NewBag.add(get(i));
		}
		return NewBag; 	 
	}
